...
const DESKTOP_SECTIONS: { label: string; items: { href: string; icon: LucideIcon; label: string }[] }[] = [
  {
    label: 'Accueil',
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/supporter-card', icon: CreditCard,  label: 'Carte Supporter' },
    ],
  },
]

// ── Mobile bottom nav (permanent, 4 slots + center PACKS) ─────────────────
...
const DRAWER_SECTIONS: { label: string; items: { href: string; icon: LucideIcon; label: string }[] }[] = [
  {
    label: 'Compétition',
    items: [
      { href: '/leaderboard', icon: Trophy,    label: 'Classement' },
...
const ALL_DRAWER_HREFS = DRAWER_SECTIONS.flatMap((s) => s.items.map((i) => i.href))
...
export function Sidebar({ user }: SidebarProps) {
  ...
  const isMenuActive =
    ALL_DRAWER_HREFS.some((href) => pathname === href || pathname.startsWith(href + '/')) ||
  ...
  return (
    <>
      {/* ── Desktop Sidebar ─────────────────────────────────────────────── */}
      <aside className="hidden lg:flex flex-col fixed left-0 top-0 h-screen w-64 border-r border-white/5 z-40 py-6 px-4 overflow-y-auto" style={{ background: 'var(--bg-sidebar)' }}>
        {/* Logo */}
        ...
        {/* User card */}
        <div className="flex items-center gap-3 mb-6 px-2 py-3 rounded-xl bg-white/5 border border-white/5">
          ...
          {user.is_vip && (
            <Crown size={14} className="text-[#F5C518] flex-shrink-0" />
          )}
        </div>
        ...
        <nav className="flex-1 space-y-5">
          {DESKTOP_SECTIONS.map((section) => (
            <div key={section.label}>
              <p className="text-white/25 text-[10px] font-black uppercase tracking-widest px-3 mb-1">
                {section.label}
              </p>
        ...
        {/* Admin */}
        {user.is_admin && (
          <Link href="/admin" className="flex items-center gap-3 px-3 py-2.5 mt-4 rounded-xl bg-purple-500/10 border border-purple-500/20 text-purple-400 hover:bg-purple-500/20 transition-colors text-sm font-bold">
            <Shield size={16} />
            Administration
          </Link>
        )}
        ...
        {drawerOpen && (
          <>
            ...
            <motion.div
              ...
              className="lg:hidden fixed bottom-0 left-0 right-0 z-50 rounded-t-3xl border-t border-white/10 max-h-[88vh] overflow-y-auto"
              style={{ background: 'var(--bg-sidebar)', paddingBottom: 'max(env(safe-area-inset-bottom), 20px)' }}
            >
              {/* Handle + user */}
              <div className="sticky top-0 z-10 px-5 pt-4 pb-3 border-b border-white/5" style={{ background: 'var(--bg-sidebar)' }}>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {user.is_vip && (
                          <span className="text-[9px] font-black px-1.5 py-0.5 rounded bg-[#F5C518]/20 text-[#F5C518]">VIP</span>
                        )}
                      </div>
                      <CoinDisplay amount={user.coins} size="sm" animated={false} />
                    </div>
                  ...
                  <button
                    onClick={() => setDrawerOpen(false)}
                    className="w-8 h-8 flex items-center justify-center rounded-full bg-white/5 text-gray-400 hover:bg-white/10"
                  >
                    <X size={16} />
                  </button>
                </div>
              ...
              <div className="px-4 py-3 space-y-4">
                ...
                {user.is_admin && (
                  <Link
                    href="/admin"
                    className="flex items-center gap-3 px-4 py-3.5 rounded-2xl bg-purple-500/15 border border-purple-500/30 text-purple-300 hover:bg-purple-500/25 active:scale-98 transition-all"
                  >
                ...
                {DRAWER_SECTIONS.map((section) => (
                  <div key={section.label}>
                    <p className="text-white/25 text-[10px] font-black uppercase tracking-widest px-1 mb-2">
                      {section.label}
                    </p>
